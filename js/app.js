...
$(function(){
     ...
     * Events:
     *   insytio-app:loaded - fires after pjax request is made and ALL scripts are trully loaded
     *   insytio-app:content-resize - fires when .content changes its size (e.g. switching between static & collapsing
     *     navigation states)
     * @constructor
     */
    var insytioAppView = function(){
        ...
        this.settings = window.insytioSettings;
        ...
        ('ontouchstart' in window) && this.$content.swipe({
            ...
            threshold: insytio.isScreen('sm') ? 100 : 200
        });
        ...
        if (this.pjaxEnabled){
            ...
            $(document).on('insytio-app:loaded', $.proxy(this._loadingFinished, this));
            $(document).on('insytio-app:loaded', $.proxy(this._collapseNavIfSmallScreen, this));
            $(document).on('insytio-app:loaded', $.proxy(this.hideLoader, this));
     ...
     * @private
     */
    insytioAppView.prototype._initOnResizeCallbacks = function(){
        var resizeTimeout,
            view = this;

        $(window).on('resize insytio-app:content-resize', function() {
     ...
     * @private
     */
    insytioAppView.prototype._initOnScreenSizeCallbacks = function(){
        var resizeTimeout,
            view = this,
            prevSize = insytio.getScreenSize();

        $(window).resize(function() {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(function(){
                var size = insytio.getScreenSize();
    ...
    insytioAppView.prototype._resetResizeCallbacks = function(){
     ...
     */
    insytioAppView.prototype.checkNavigationState = function(){
        if (this.isNavigationStatic()){
            this.staticNavigationState();
            if (insytio.isScreen('md') || insytio.isScreen('sm') || insytio.isScreen('xs')){
                this.collapseNavigation();
            }
        } else {
            if (insytio.isScreen('lg') || insytio.isScreen('xl')){
                var view = this;
                setTimeout(function(){
     ...
     */
    insytioAppView.prototype.toggleNavigationCollapseState = function(){
    ...
    insytioAppView.prototype.collapseNavigation = function(){
        //this method only makes sense for non-static navigation state
        if (this.isNavigationStatic() && (insytio.isScreen('lg') || insytio.isScreen('xl'))) return;
    ...
    insytioAppView.prototype.expandNavigation = function(){
        //this method only makes sense for non-static navigation state
        if (this.isNavigationStatic() && (insytio.isScreen('lg') || insytio.isScreen('xl'))) return;
    ...
    insytioAppView.prototype._sidebarMouseEnter = function(){
        if (insytio.isScreen('lg') || insytio.isScreen('xl')){
            this.expandNavigation();
        }
    };

    insytioAppView.prototype._sidebarMouseLeave = function(){
        if (insytio.isScreen('lg') || insytio.isScreen('xl')){
            this.collapseNavigation();
        }
    };

    insytioAppView.prototype._collapseNavIfSmallScreen = function(){
        if (insytio.isScreen('xs') || insytio.isScreen('sm') || insytio.isScreen('md')){
            this.collapseNavigation();
        }
    };
     ...
     */
    insytioAppView.prototype.toggleNavigationState = function(){
        if (this.isNavigationStatic()){
            this.collapsingNavigationState();
        } else {
            this.staticNavigationState();
        }
        $(window).trigger('insytio-app:content-resize');
    };
     ...
     */
    insytioAppView.prototype.staticNavigationState = function(){
     ...
     * Static navigation state - navigation stays always open.
     */
    insytioAppView.prototype.collapsingNavigationState = function(){
        this.settings.set('nav-static', false).save();
        $('body').removeClass('nav-static');
        this.collapseNavigation();
    };

    insytioAppView.prototype.isNavigationStatic = function(){
     ...
     * @private
     */
    insytioAppView.prototype._changeActiveNavigationItem = function(event, xhr, options){
     ...
     * @private
     */
    insytioAppView.prototype._contentSwipeLeft = function(){
        //this method only makes sense for small screens + ipad
        if (insytio.isScreen('xl')) return;
     ...
     * @private
     */
    insytioAppView.prototype._contentSwipeRight = function(){
        //this method only makes sense for small screens + ipad
        if (insytio.isScreen('xl')) return;

        // fixme. this check is bad. I know. breaks loose coupling principle
        // insytioApp should not know about some "strange" sidebar chat.
    ...
    insytioAppView.prototype.showLoader = function(){
        var view = this;
    ...
    insytioAppView.prototype.hideLoader = function(){
     ...
     */
    insytioAppView.prototype.onResize = function(fn, /**Boolean=*/ allPages){
     ...
     * @param fn A function to execute
     */
    insytioAppView.prototype.onPageLoad = function(fn){
     ...
     */
    insytioAppView.prototype.onScreenSize = function(size, fn, /**Boolean=*/ onEnter){
     ...
     */
    insytioAppView.prototype.pageLoaded = function(){
     ...
     * @private
     */
    insytioAppView.prototype._addPageCallback = function(callbacks, fn){
     ...
     * @private
     */
    insytioAppView.prototype._runPageCallbacks = function(callbacks){
        var pageName = this.extractPageName(location.href);
        if (callbacks[pageName]){
     ...
     * @private
     */
    insytioAppView.prototype._loadScripts = function(event, data, status, xhr, options){
        ...
        var view = this;
        $previous.load(function(){
            $(document).trigger('insytio-app:loaded');
            view.log('scripts loaded.');
        })
    };

    insytioAppView.prototype.extractPageName = function(url){
    ...
    insytioAppView.prototype._checkLoading = function(e){
    ...
    insytioAppView.prototype._loadingFinished = function(){
        this.loading = false;
    };

    insytioAppView.prototype._logErrors = function(){
        var errors = JSON.parse(localStorage.getItem('lb-errors')) || {};
        errors[new Date().getTime()] = arguments;
        localStorage.setItem('insytio-errors', JSON.stringify(errors));
        this.debug && alert('check errors');
    };

    insytioAppView.prototype.log = function(message){
        if (this.debug){
            console.log("insytioApp: "
                    + message
    ...
    window.insytioApp = new insytioAppView();

//    insytioApp.expandNavigation();
...
function initAppFunctions(){
    !function($){
        ...
        insytioApp.onScreenSize(['sm','xs'], moveNotificationsDropdown);
        insytioApp.onScreenSize(['sm','xs'], moveBackNotificationsDropdown, false);

        insytio.isScreen('sm') && moveNotificationsDropdown();
        insytio.isScreen('xs') && moveNotificationsDropdown();
        ...
        insytioApp.onResize(initSidebarScroll, true);
        initSidebarScroll();
    ...
    !function($){
        ...
        $(document).on('click', '[data-toggle=chat-sidebar]', function(){
            ...
            $(this).find('.chat-notification-insytio').remove();
        });
        ...
        $('#content').on('swipeLeft', function(e){
            ...
            if ($('body').is('.nav-collapsed.chat-sidebar-opened')){
                $chatContainer.removeClass('chat-sidebar-opened')
                    ...
                    // used by insytioApp to check whether it is permitted to open navigation
                    // on swipeRight
        ...
        insytioApp.onResize(initChatSidebarScroll, true);
...
/**
 * insytio browser fixes. It's always something broken somewhere
 */
...
/**
 * Demo-only functions. Does not affect the core insytio functionality.
...
function initDemoFunctions(){
    !function($){
        ...
        setTimeout(function(){
            ...
            $chatNotification.siblings('[data-toggle="chat-sidebar"]').append('<i class="chat-notification-insytio animated bounceIn"></i>')
        }, 4000)
